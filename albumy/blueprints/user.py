...
from albumy.forms.user import EditProfileForm, UploadAvatarForm, CropForm, ChangePasswordForm, ChangeEmailForm, \
...
def follow(username):
    ...
    flash('Followed {}!'.format(username), 'success')
...
@user_bp.route('/<username>/follwers')
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.validate_password(form.old_password.data):
            ...
            return redirect(url_for('.index', username=current_user.username))
        flash('Incorrect old password.', 'info')
...
def change_email_request():
    ...
    if form.validate_on_submit():
        token = generate_token(user=current_user, operation=Operation.CHANGE_EMAIL, new_email=form.email.data)
...
def change_email(token):
    if validate_token(token=token, user=current_user, operation=Operation.CHANGE_EMAIL):
